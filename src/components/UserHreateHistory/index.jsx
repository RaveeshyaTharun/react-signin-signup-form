...
const UserHeartDiseaseHistory = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const response = await axios.get('http://localhost:5000/history-heart-disease');
        ...
        <Paper elevation={3} style={{ padding: '20px', marginTop: '20px' }}>
          ...
          <TableContainer>
            <Table>
              <TableHead>
                <TableRow>
                  <TableCell><strong>Date & Time (Sri Lanka)</strong></TableCell>
              ...
              <TableBody>
                {history.map((record, index) => (
                  <TableRow key={index}>
                    <TableCell align="center">{formatDateTime(record.date)}</TableCell>
